...
static long kernfs_dir_ioctl (struct file * filp, unsigned int n, unsigned long m) {
	struct kernfs_node *kn = kernfs_node_from_dentry(filp->f_path.dentry);
	struct kernfs_syscall_ops *scops = kernfs_root(kn)->syscall_ops;
	if(scops->unlocked_ioctl) {
		return scops->unlocked_ioctl(filp, n, m);
	}
	return -ENOIOCTLCMD;
}
...
const struct file_operations kernfs_dir_fops = {
	...
	.unlocked_ioctl = kernfs_dir_ioctl,
};
